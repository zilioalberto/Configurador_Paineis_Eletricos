...
def _aplicar_detalhes_icms(
    detalhes: dict,
    *,
    icms_entradas: Decimal | None,
    icms_saidas: Decimal | None,
    limpar: bool,
) -> None:
...
def _persistir_detalhes_manual(
    ...
    *,
    ...
    if tipo == TipoReconciliacaoFiscalChoices.ICMS:
        _aplicar_detalhes_icms(
            detalhes,
            icms_entradas=icms_entradas,
            icms_saidas=icms_saidas,
            limpar=limpar,
        )
...
def _atualizar_das_inss_manual(pacote: PacoteObrigacaoFiscal, tipo: str, valor: Decimal | None) -> None:
...
def _atualizar_icms_manual(
    ...
    *,
    ...
    entradas = icms_entradas if icms_entradas is not None else (
        snapshot.valor_contabil_entradas or valor_icms_manual(pacote, "entradas")
    )
    saidas = icms_saidas if icms_saidas is not None else (
        snapshot.valor_contabil_saidas or valor_icms_manual(pacote, "saidas")
    )
    ...
    _persistir_detalhes_manual(pacote, tipo, valor=saidas, icms_entradas=entradas, icms_saidas=saidas)


def _atualizar_obrigacao_manual(pacote: PacoteObrigacaoFiscal, tipo: str, valor: Decimal | None) -> None:
    if valor is None or valor <= 0:
...
def atualizar_contabilidade_manual(
    ...
    *,
    ...
    if tipo == TipoReconciliacaoFiscalChoices.DAS_INSS:
        _atualizar_das_inss_manual(pacote, tipo, valor)
        return

    if tipo == TipoReconciliacaoFiscalChoices.ICMS:
        _atualizar_icms_manual(pacote, tipo, icms_entradas=icms_entradas, icms_saidas=icms_saidas)
        return

    _atualizar_obrigacao_manual(pacote, tipo, valor)
