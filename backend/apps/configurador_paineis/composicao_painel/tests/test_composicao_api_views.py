...
class TestComposicaoGerarSugestoesView:
    @patch("apps.configurador_paineis.composicao_painel.api.views.gerar_sugestoes_painel")
...
class TestComposicaoReavaliarPendenciasView:
    @patch("apps.configurador_paineis.composicao_painel.api.views.reavaliar_pendencias_projeto")
...
class TestSugestaoAlternativasView:
    @patch("apps.configurador_paineis.composicao_painel.api.views.listar_alternativas_para_sugestao", return_value=[])
...
class TestComposicaoItemReabrirView:
    ...
    @patch(
        "apps.configurador_paineis.composicao_painel.api.views.reabrir_composicao_item_para_sugestao"
    )
    def test_integrity_error_retorna_409_amigavel(
        self, mock_reabrir, admin_client, criar_projeto
    ):
        from django.db import IntegrityError

        mock_reabrir.side_effect = IntegrityError("UNIQUE constraint failed")
        client, _ = admin_client
        projeto = criar_projeto(nome="ReC", codigo="09702-26", tensao_nominal=TensaoChoices.V380)
        produto = Produto.objects.create(
            codigo="RE-C1",
            descricao="Conflito",
            categoria=CategoriaProdutoNomeChoices.CONTATORA,
