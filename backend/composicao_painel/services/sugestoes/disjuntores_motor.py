...
def processar_sugestao_disjuntor_motor_para_carga(
    ...
    """
    Gera ou atualiza sugestão/pendência de disjuntor motor para uma única carga MOTOR.
    """
    print("-" * 100)
    print(f"[DISJUNTORES MOTOR] Processando carga: id={carga.id} | carga={carga}")

    if carga.tipo != TipoCargaChoices.MOTOR:
        print("[DISJUNTORES MOTOR] Carga não é MOTOR. Pulando.")
        return None
...
def gerar_sugestoes_disjuntores_motor(projeto):
    """
    Gera sugestões de disjuntores motor para todas as cargas MOTOR elegíveis.
    Remove antes todas as sugestões/pendências de disjuntor motor (motor) do projeto.
    """
    print("\n" + "=" * 100)
