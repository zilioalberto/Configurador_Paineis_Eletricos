...
def test_reavaliar_contatora_com_carga_chama_reprocessar(
    ...
    with patch(
        "configurador.composicao_painel.services.sugestoes.orquestrador_pendencias.reprocessar_contatora_para_carga"
    ) as mock_r:
...
def test_reavaliar_rele_sobrecarga_com_carga_chama_reprocessar(
    ...
    with patch(
        "configurador.composicao_painel.services.sugestoes.orquestrador_pendencias.reprocessar_rele_sobrecarga_para_carga"
    ) as mock_r:
        mock_r.return_value = None
        r = reavaliar_pendencias_projeto(projeto)
    mock_r.assert_called_once_with(projeto, carga)
...
def test_reavaliar_fusivel_com_carga_chama_reprocessar(
    ...
    with patch(
        "configurador.composicao_painel.services.sugestoes.orquestrador_pendencias.reprocessar_fusivel_para_carga"
    ) as mock_r:
        mock_r.return_value = None
        r = reavaliar_pendencias_projeto(projeto)
    mock_r.assert_called_once_with(projeto, carga)
...
def test_reavaliar_seccionamento_chama_reprocessar(criar_projeto):
    ...
    with patch(
        "configurador.composicao_painel.services.sugestoes.orquestrador_pendencias.reprocessar_seccionamento_para_pendencia"
    ) as mock_s:
