...
from configurador.cargas.models import Carga, CargaResistencia
from catalogo.models import EspecificacaoDisjuntorCaixaMoldada, Produto
from configurador.composicao_painel.models import SugestaoItem
from configurador.composicao_painel.services.alternativas_produto import (
...
def test_listar_alternativas_contatora_sem_corrente_retorna_vazio():
    ...
    s.projeto.tipo_corrente_comando = "CC"
    with patch(
        "configurador.composicao_painel.services.alternativas_produto._corrente_referencia_sugestao",
        return_value=None,
    ):
        assert not listar_alternativas_para_sugestao(s).exists()
...
def test_listar_alternativas_disjuntor_caixa_moldada_field_error_retorna_vazio():
    ...
    s.carga_id = None
    with patch(
        "configurador.composicao_painel.services.alternativas_produto.selecionar_disjuntores_caixa_moldada",
...
def test_listar_alternativas_contatora_delega_selecionar_contatoras():
    ...
    with patch(
        ...
        with patch(
            "configurador.composicao_painel.services.alternativas_produto.selecionar_contatoras",
...
def test_listar_alternativas_disjuntor_motor_delega_selecionar():
    ...
    with patch(
        ...
        with patch(
            "configurador.composicao_painel.services.alternativas_produto.selecionar_disjuntores_motor",
            return_value=fake_qs,
        ) as m_sel:
            assert listar_alternativas_para_sugestao(s) is fake_qs
...
def test_listar_alternativas_rele_sobrecarga_delega_selecionar():
    ...
    with patch(
        ...
        with patch(
            "configurador.composicao_painel.services.alternativas_produto.selecionar_reles_sobrecarga",
            return_value=fake_qs,
        ) as m_sel:
            assert listar_alternativas_para_sugestao(s) is fake_qs
    call_kw = m_sel.call_args.kwargs
...
def test_listar_alternativas_fusivel_delega_selecionar():
    ...
    with patch(
        ...
        with patch(
            "configurador.composicao_painel.services.alternativas_produto.selecionar_fusiveis",
            return_value=fake_qs,
        ) as m_sel:
            assert listar_alternativas_para_sugestao(s) is fake_qs
    call_kw = m_sel.call_args.kwargs
...
def test_listar_alternativas_seccionadora_delega_selecionar():
    ...
    fake_qs = Produto.objects.none()
    with patch(
        "configurador.composicao_painel.services.alternativas_produto.selecionar_seccionadoras",
        return_value=fake_qs,
    ) as m_sel:
        assert listar_alternativas_para_sugestao(s) is fake_qs
