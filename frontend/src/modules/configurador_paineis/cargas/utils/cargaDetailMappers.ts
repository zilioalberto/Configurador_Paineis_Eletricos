...
export function mapMotorDetail(d: CargaDetail) {
  if (d.tipo !== 'MOTOR') return undefined
  if (!d.motor) return defaultMotor()

  const u = String(d.motor.potencia_corrente_unidade ?? 'CV')
  const unidade: 'CV' | 'KW' | 'A' =
    u === 'KW' || u === 'A' || u === 'CV' ? u : 'CV'
  return {
    ...
    tipo_conexao_painel: str(d.motor.tipo_conexao_painel, 'CONEXAO_BORNES_COM_PE'),
  }
}

export function mapValvulaDetail(d: CargaDetail) {
  if (d.tipo !== 'VALVULA') return undefined
  if (!d.valvula) return defaultValvula()
...
export function mapResistenciaDetail(d: CargaDetail) {
  if (d.tipo !== 'RESISTENCIA') return undefined
  if (!d.resistencia) return defaultResistencia()
...
export function mapSensorDetail(d: CargaDetail) {
  if (d.tipo !== 'SENSOR') return undefined
  if (!d.sensor) return defaultSensor()
  ...
  return {
    ...
    quantidade_fios:
      str(d.sensor.quantidade_fios) === ''
        ? ('' as const)
        : num(d.sensor.quantidade_fios, 0),
...
export function mapTransdutorDetail(d: CargaDetail) {
  if (d.tipo !== 'TRANSDUTOR') return undefined
  if (!d.transdutor) return defaultTransdutor()

  return {
    tipo_transdutor: str(d.transdutor.tipo_transdutor, 'PRESSAO'),
    faixa_medicao: str(d.transdutor.faixa_medicao, ''),
    tipo_sinal_analogico: str(d.transdutor.tipo_sinal_analogico, 'CORRENTE_4_20MA'),
    ...
    quantidade_fios:
      str(d.transdutor.quantidade_fios) === ''
        ? ('' as const)
        : num(d.transdutor.quantidade_fios, 0),
  }
}
