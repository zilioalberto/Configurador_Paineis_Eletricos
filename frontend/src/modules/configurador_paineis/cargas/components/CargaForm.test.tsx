...
describe('CargaForm', () => {
  ...
  it('oculta requisitos e I/O quando o projeto não tem PLC', () => {
    const initial = cargaFormInitial('proj-1')
    initial.descricao = 'Alguma'
    initial.tag = 'X1'
    render(ui(initial))
    expect(screen.queryByLabelText(/Exige comando/i)).not.toBeInTheDocument()
    expect(
      screen.queryByText(/Ocupação de entradas e saídas digitais\/analógicas só pode ser/i)
