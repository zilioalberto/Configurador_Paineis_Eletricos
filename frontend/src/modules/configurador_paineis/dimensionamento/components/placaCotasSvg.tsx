...
/** Cotas horizontais empilhadas abaixo da placa (segmentos contíguos). */
export function CotasHorizontaisEmpilhadas({
  segmentos,
  yPlaca,
  yCota,
  tx,
}: {
  segmentos: SegmentoCotaMm[]
  yPlaca: number
  yCota: number
  tx: (mm: number) => number
}) {
  return (
    <g>
      {segmentos.map((seg, i) => (
        <CotaLinear
          key={`h-seg-${i}-${seg.inicio_mm}`}
          x1={tx(seg.inicio_mm)}
          y1={yCota}
          x2={tx(seg.fim_mm)}
          y2={yCota}
          refX1={tx(seg.inicio_mm)}
          refY1={yPlaca}
          refX2={tx(seg.fim_mm)}
          refY2={yPlaca}
          label={seg.label}
...
/** Cotas verticais empilhadas à esquerda da placa. */
export function CotasVerticaisEmpilhadas({
  segmentos,
  xPlaca,
  xCota,
  ty,
