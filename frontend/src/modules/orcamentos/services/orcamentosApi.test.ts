...
describe('orcamentosApi', () => {
  ...
  it('obtem orcamento por id', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/orcamentos/o-1/')
  })
  ...
  it('cria orçamento sem enviar código manual', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/orcamentos/', {
      titulo: 'Nova proposta',
      cliente: 'cliente-1',
      contato_cliente: 'contato-1',
    })
  ...
  it('atualiza orcamento em PATCH parcial', async () => {
    ...
    expect(patchMock).toHaveBeenCalledWith('/orcamentos/o-1/', {
      titulo: 'Novo',
      status: 'ENVIADO',
    })
  })

  it('atualiza oferta com margens e valores atuais do catálogo', async () => {
    postMock.mockResolvedValueOnce({
      data: {
        itens_atualizados: 2,
        orcamento: { id: 'o-1', titulo: 'Oferta', status: 'RASCUNHO', itens: [] },
      },
    })

    await expect(atualizarOfertaOrcamento('o-1')).resolves.toEqual({
      itens_atualizados: 2,
      orcamento: { id: 'o-1', titulo: 'Oferta', status: 'RASCUNHO', itens: [] },
    })

    expect(postMock).toHaveBeenCalledWith('/orcamentos/o-1/atualizar-oferta/')
  })

  it('reabre oferta finalizada para rascunho', async () => {
    postMock.mockResolvedValueOnce({
      data: { id: 'o-1', titulo: 'Oferta', status: 'RASCUNHO', editavel: true },
    })

    await expect(reabrirOfertaOrcamento('o-1')).resolves.toMatchObject({
      status: 'RASCUNHO',
      editavel: true,
    })

    expect(postMock).toHaveBeenCalledWith('/orcamentos/o-1/reabrir/')
  })

  it('revisa preço de catálogo a partir do item do orçamento', async () => {
    postMock.mockResolvedValueOnce({
      data: { id: 'o-1', titulo: 'Oferta', itens: [{ id: 'i-1', custo_unitario: '150.00' }] },
    })

    await expect(
      revisarPrecoCatalogoItemOrcamento('o-1', 'i-1', '150.00', 'Cotação revisada')
  ...
  it('lista orcamentos', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/orcamentos/')
  })
  ...
  it('lista margens de clientes como array ou como paginacao', async () => {
    ...
    expect(getMock).toHaveBeenCalledWith('/orcamentos/margens-clientes/')
  })
  ...
  it('cria e atualiza margem de cliente', async () => {
    ...
    expect(postMock).toHaveBeenCalledWith('/orcamentos/margens-clientes/', {
      cliente: 'c-1',
      margem_produtos_percentual: '12',
    })
    expect(patchMock).toHaveBeenCalledWith('/orcamentos/margens-clientes/m-1/', {
      margem_produtos_percentual: '15',
    })
  })
  ...
  it('cria nova revisão e gerencia painéis do configurador', async () => {
    ...
    expect(postMock).toHaveBeenNthCalledWith(1, '/orcamentos/o-1/nova-revisao/', {
      tipo_revisao: 'TECNICA',
      paineis_reconfigurar: ['vp-1'],
    })
    expect(postMock).toHaveBeenNthCalledWith(2, '/orcamentos/o-1/configuradores-painel/', {
      descricao_painel: 'Painel 02',
    })
    expect(postMock).toHaveBeenNthCalledWith(
      3,
      '/orcamentos/o-1/configuradores-painel/vp-1/iniciar-configurador/'
    )
    expect(postMock).toHaveBeenNthCalledWith(
      4,
      '/orcamentos/o-1/configuradores-painel/vp-1/vincular-projeto/',
      { projeto_configurador_id: 'proj-99' }
    )
    expect(postMock).toHaveBeenNthCalledWith(
      5,
      '/orcamentos/o-1/configuradores-painel/vp-1/sincronizar-composicao/'
    )
  })
})
